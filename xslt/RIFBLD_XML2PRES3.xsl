...
<xsl:stylesheet 
   ...
   -->
     ...
     <xsl:template match="rif:Import">
     	...
     	Import ( 
     		<xsl:apply-templates  select="rif:location"/>
     		 <xsl:text> </xsl:text> <!-- to add space. whitespace code (e.g &#32;) might not work  --> 
    ...
    <!--
  	 ...
  	 -->
  	  <xsl:template name="rif:UNITERM">	
      	...
      	(
      	<xsl:choose>	        	   		
		    <xsl:when test="rif:args">		   		
				<xsl:apply-templates select="rif:args"/>
		    </xsl:when>
		   <xsl:when test="rif:slot">		   		
    ...
    <!-- 
     ...
     -->
     <xsl:template name="rif:slot" mode="slot-UNITERM">
     	<xsl:apply-templates select="rif:Name"/>
     	->
     	<xsl:call-template name="rif:TERM"/>
     </xsl:template>
    ...
    <!-- 
     ...
     <xsl:template name="rif:TERM">
     	<xsl:choose>	        	   		
		    <xsl:when test="rif:Const">		   		
				<xsl:apply-templates select="rif:Const"/>
		    </xsl:when>
		   <xsl:when test="rif:Var">		   		
				<xsl:apply-templates select="rif:Var"/>
		    </xsl:when>	
		     <xsl:when test="rif:Expr">		   		
				<xsl:apply-templates select="rif:Expr"/>
		    </xsl:when>	
		     <xsl:when test="rif:List">		   		
    ...
    <!-- 
     ...
     <xsl:template match="rif:External" mode="External-TERM">
     	<!-- check for IRIMETA tag -->
     	External (
     	<xsl:apply-templates match="rif:content" mode="content-TERM"/>
     	)
     </xsl:template>
